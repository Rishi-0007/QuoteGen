...
export default function AppShell({ children }) {
  ...
  // close drawer on route change (e.g., Dashboard/New Quote click)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-background text-white flex">
      ...
      >
        ...
        {/* Drawer */}
        <div
          className={`absolute top-0 left-0 h-full w-[85%] max-w-80 bg-zinc-950 border-r border-white/10 transform transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full"}`}
          role="dialog"
          aria-modal="true"
        >
          ...
          <div className="overflow-y-auto h-[calc(100%-3.5rem)] drawer-sidebar" onClick={handleNavClick}>
            <Sidebar />
          </div>
        </div>
      </div>

...
export default function AccommodationItem({ index, form }) {
  ...
  return (
    <div className="grid md:grid-cols-4 gap-3">
      ...
      <div className="md:col-span-2"><div className="label">Hotel / Property</div>
        <select className="input" {...form.register(`items.${index}.hotelProperty`)}>
          <option value="">Select a property</option>
          {options.map((p)=> <option key={p} value={p}>{p}</option>)}
        </select>
      </div>
      <div><div className="label">Room(s)</div><Input type="number" step="1" min="1" {...form.register(`items.${index}.roomCount`)} placeholder="e.g., 1" /></div>
      <div className="md:col-span-2"><div className="label">Room Details</div><Input {...form.register(`items.${index}.roomDetails`)} placeholder="e.g., Deluxe Room with Sea View" /></div>
      <div><div className="label">Adults</div><Input type="number" step="1" {...form.register(`items.${index}.adults`)} placeholder="2" /></div>
      <div><div className="label">Children</div><Input type="number" step="1" {...form.register(`items.${index}.children`)} placeholder="0" /></div>
      <div><div className="label">Guests (total)</div><Input type="number" step="1" {...form.register(`items.${index}.guests`)} placeholder="2" /></div>

...
const BRANDS = {
  holidays_seychelle: { name: "Holidays Seychelle", emergency: "+248 437 6666", logo: "/brands/HS-logo.png" },
...
const DEFAULT_NOTES = [
  "The hotel owner or experience provider may request a copy of your passport upon check in.",
  "To avoid unnecessary charges please ensure that you are familiar with the service providers’ cancellation policy.",
  "We strive to provide you with a quality service. If you see yourself not getting the service that you deserve please do give a call to our emergency number provided.",
];
...
function buildNotesHtml(quote) {
  if (quote.notesPreset === "custom" && quote.notesCustom && quote.notesCustom.trim().length > 0) {
    const lines = quote.notesCustom.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
    const bullets = lines.map(l => `<li>${escapeHtml(l.replace(/^[-*]\s*/, ""))}</li>`).join("");
    return `<ul>${bullets}</ul>`;
  }
  return `<ul>${DEFAULT_NOTES.map(n => `<li>${escapeHtml(n)}</li>`).join("")}</ul>`;
}
export function quoteToHtml(quote) {
  ...
  const rows = (quote.items || []).slice().sort((a, b) => {
    ...
    if (it.type === "accommodation") {
      ...
      name = it.itemTitle || "Activity"; details = it.description || ""; date = fmt(it.startDate);
    } else {
    ...
    return { name: escapeHtml(name), subline: escapeHtml(subline), details: escapeHtml(details), cancel: escapeHtml(cancellationNote), date: escapeHtml(date), price: escapeHtml(money(total, cur)) };
  });
  ...
  <body><div class="page">
    ...
    <div class="section card"><div class="title">Agent Details</div><div class="kv">
      <div>Contact Person</div><div>Vacations & More</div>
    ...
    <div class="section notes"><div class="title">Important Notes</div>${notesHtml}</div>

...
const BRANDS = {
  holidays_seychelle: {
    name: "Holidays Seychelle",
    emergency: "+248 437 6666",
    logo: "" // optional: /brands/holidays_seychelle.png
  },
  oceanic_travel: { name: "Oceanic Travel Co.", emergency: "+1 555 0100", logo: "" },
  sunrise_journeys: { name: "Sunrise Journeys", emergency: "+1 555 0110", logo: "" },
};

const fmt = (d) => (d ? new Date(d).toLocaleDateString() : "");
const money = (v, cur = "INR") => `${cur} ${(Number(v || 0)).toFixed(2)}`;

// Parse "Hotel (Location)" into { name, location }
...
export function quoteToHtml(quote) {
  ...
  const brandKey = quote.footerBrand || "holidays_seychelle";
  const brand = BRANDS[brandKey] || BRANDS.holidays_seychelle;
  ...
  const rows = (quote.items || [])
    ...
    .map((it) => {
      ...
      let name = "";
      let subline = ""; // for hotel location line
      let details = "";
      let date = "";
      ...
      if (it.type === "accommodation") {
        ...
        const pax = [
          (it.adults || it.adults === 0) ? `${it.adults} Adults` : null,
          (it.children || it.children === 0) ? `${it.children} Children` : null,
          (it.guests || it.guests === 0) ? `${it.guests} Guests` : null,
        ].filter(Boolean).join(", ");
        details = [rn, it.roomDetails, pax].filter(Boolean).join(" ");
        date = `${fmt(it.checkIn)} - ${fmt(it.checkOut)}`;
      } else if (it.type === "transfer") {
        const label =
          it.transferType === "ferry" ? "Ferry Transfer" :
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    body { margin:0; padding:0; font:12px/1.45 -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial; color:var(--text); background:var(--bg); }
    .page { padding:24px 20px; }
    /* Minimal header */
    .header { padding:8px 0; border-bottom:1px solid var(--line); }
    .header h1 { margin:0; font-size:20px; color:var(--accent); letter-spacing:.2px; }
    .header .tag { margin-top:4px; font-size:12px; color:var(--muted); }
    .section { margin-top:14px; }
    .title { font-weight:700; margin-bottom:6px; }
    .bookingTitle { font-size:14px; font-weight:700; margin:10px 0 6px; }
    .card { border:1px solid var(--line); background:var(--bg-soft); border-radius:10px; padding:10px 12px; }
    ...
    .subline { display:block; font-size:10px; color:var(--muted); margin-top:2px; }
    .cancel-note { margin-top:4px; font-size:10px; color:var(--muted); }
    .summary { width:100%; margin-top:8px; display:grid; grid-template-columns:1fr 200px; gap:6px; }
    .summary .label{ color:var(--muted); } .summary .value{ text-align:right; }
    .notes{ border:1px dashed var(--line); background:#fff; border-radius:10px; padding:10px 12px; }
    .notes ul{ margin:6px 0 0 18px; padding:0; } .notes li{ margin:4px 0; }
    .footer{ margin-top:12px; display:flex; align-items:center; justify-content:space-between; gap:10px; color:var(--muted); }
    .footer img{ height:28px; width:auto; object-fit:contain; }
  </style>
</head>
<body>
  <div class="page">
    ...
    <div class="section card">
      ...
      <div class="kv">
        <div>Contact Person</div><div>Piyush Bhardwaj</div>
        <div>Mobile Number</div><div>8199050506</div>
        <div>E Mail</div><div>director@evrenglobalsolutions.com</div>
        <div>Subject</div><div>Quotation for your travel Itinerary</div>
      </div>
    </div>

    <div class="section">
      <div>Dear Sir/Ma'am,</div>
      <div class="muted">Please find below the details of the prices for the travel items as per your requirements.</div>
    </div>

    <div class="section">
      <div class="bookingTitle">Booking Details</div>
      <table class="table">
        <thead><tr><th>Name</th><th>Details</th><th>Date</th><th>Pricing</th></tr></thead>
        <tbody>
          ${rows.map(r => `
            <tr>
    ...
    <div class="section notes">
      ...
      <ul>
        <li>The hotel owner or experience provider may request a copy of your passport upon check in.</li>
        <li>To avoid unnecessary charges please ensure that you are familiar with the service providers’ cancellation policy.</li>
        <li>We strive to provide you with a quality service. If you see yourself not getting the service that you deserve please do give a call to our emergency number provided.</li>
      </ul>
    </div>
